...
export default function SettingsPage() {
  ...
  // ==========================================
  // STATE: TEMPAT SAMPAH (TRASH) BARU!
  // ==========================================
  ...
  // Load avatar & WA dari local storage pas pertama kali render
  useEffect(() => {
  ...
  const fetchTrashData = async () => {
    ...
    // Tarik data yang is_deleted-nya TRUE
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Refresh daftar sampah kalau barusan melakukan restore
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-12 w-full max-w-6xl mx-auto relative">
      ...
      {/* HEADER */}
      <div className="mb-8">
        <h1 className="text-[26px] font-bold text-gray-900 tracking-tight">Pengaturan Sistem</h1>
        <p className="text-[13px] text-gray-500 font-medium mt-1">Kelola konfigurasi WhatsApp, Profil Perusahaan, Basis Data, dan Pemulihan.</p>
      </div>

      <div className="flex flex-col md:flex-row gap-8 items-start">
        
        {/* === SIDEBAR KIRI (TAB MENU) === */}
        <div className="w-full md:w-[240px] shrink-0 bg-white border border-gray-200 rounded-[16px] p-2 shadow-sm flex flex-col gap-1 sticky top-24">
          <button onClick={() => setActiveTab("whatsapp")} className={`flex items-center gap-3 px-4 py-3 rounded-[10px] text-[13px] font-bold transition-all text-left ${activeTab === "whatsapp" ? "bg-black text-white" : "text-gray-600 hover:bg-gray-100"}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
            WhatsApp & Notifikasi
          </button>
          <button onClick={() => setActiveTab("company")} className={`flex items-center gap-3 px-4 py-3 rounded-[10px] text-[13px] font-bold transition-all text-left ${activeTab === "company" ? "bg-black text-white" : "text-gray-600 hover:bg-gray-100"}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
            Profil Perusahaan
          </button>
          <button onClick={() => setActiveTab("data")} className={`flex items-center gap-3 px-4 py-3 rounded-[10px] text-[13px] font-bold transition-all text-left ${activeTab === "data" ? "bg-black text-white" : "text-gray-600 hover:bg-gray-100"}`}>
          ...
          <button onClick={() => setActiveTab("trash")} className={`flex items-center gap-3 px-4 py-3 rounded-[10px] text-[13px] font-bold transition-all text-left ${activeTab === "trash" ? "bg-red-50 text-red-600 border border-red-100" : "text-gray-600 hover:bg-red-50 hover:text-red-600"}`}>
        ...
        <div className="flex-1 w-full">
            ...
            <div className="bg-white border border-gray-200 rounded-[16px] shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 sm:p-8 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-[13px] text-gray-500 mt-1">Atur nomor pengirim dan format pesan otomatis saat membagikan dokumen PDF.</p>
                </div>
              ...
              <form onSubmit={(e) => triggerAction("wa", e)} className="p-6 sm:p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea rows={4} value={waTemplate} onChange={(e) => setWaTemplate(e.target.value)} className="w-full border border-gray-200 rounded-[8px] px-4 py-3 text-[13px] outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 font-medium resize-none transition-all"></textarea>
                ...
                <div className="flex justify-end pt-4 border-t border-gray-100">
                  <button type="submit" className="bg-[#2D68FF] hover:bg-blue-700 text-white px-8 py-2.5 rounded-[8px] text-[13px] font-bold shadow-sm transition-colors">Simpan Pengaturan WA</button>
                </div>
              </form>
            </div>
          )}
          ...
          {activeTab === "company" && (
            <div className="bg-white border border-gray-200 rounded-[16px] shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 sm:p-8 border-b border-gray-100">
                <h2 className="text-[16px] font-bold text-gray-900">Profil Perusahaan & Admin</h2>
                <p className="text-[13px] text-gray-500 mt-1">Identitas ini akan tercetak otomatis di dokumen PDF dan Top Bar sistem.</p>
              </div>
              <form onSubmit={(e) => triggerAction("company", e)} className="p-6 sm:p-8 space-y-8">
                <div className="flex items-center gap-6 bg-gray-50 p-6 rounded-[12px] border border-gray-100">
                ...
                <div className="space-y-2">
                  ...
                  <textarea rows={3} value={companyAddress} onChange={(e) => setCompanyAddress(e.target.value)} className="w-full border border-gray-200 rounded-[8px] px-4 py-3 text-[13px] outline-none focus:border-black font-medium resize-none"></textarea>
                </div>

                <div className="flex justify-end pt-4 border-t border-gray-100">
                  <button type="submit" className="bg-black hover:bg-gray-800 text-white px-8 py-2.5 rounded-[8px] text-[13px] font-bold shadow-sm transition-colors">Simpan Profil</button>
                </div>
              </form>
            </div>
          )}
          ...
          {activeTab === "data" && (
            <div className="bg-white border border-gray-200 rounded-[16px] shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 sm:p-8 border-b border-gray-100">
                <h2 className="text-[16px] font-bold text-gray-900">Manajemen Data & Cache</h2>
                <p className="text-[13px] text-gray-500 mt-1">Ruang kontrol database untuk backup data dan mencegah penumpukan log.</p>
              </div>
              
              <div className="p-6 sm:p-8 space-y-6">
                <div className="border border-gray-200 rounded-[12px] p-6 relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 border border-blue-100 bg-blue-50/30 rounded-[12px]">
                  <div>
                    <h3 className="text-[14px] font-bold text-blue-800 flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                      Backup Seluruh Data
                    </h3>
                    <p className="text-[12px] text-gray-600 mt-1 max-w-md">Unduh seluruh data Mesin dan Work Order ke dalam format JSON lokal sebagai cadangan keamanan.</p>
                  </div>
                  <button type="button" onClick={() => triggerAction("backup")} className="shrink-0 bg-[#2D68FF] text-white hover:bg-blue-700 px-6 py-2.5 rounded-[8px] text-[12px] font-bold transition-all shadow-sm">
                    Download Backup
                  </button>
                </div>

                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 border border-red-100 bg-red-50/50 rounded-[12px]">
                  <div>
                    ...
                    <p className="text-[12px] text-gray-600 mt-1 max-w-md">Hapus semua riwayat "Recent Activity" di Dashboard. Aksi ini tidak akan menghapus data Mesin atau Work Order.</p>
                  </div>
                  <button type="button" onClick={() => triggerAction("clear_logs")} className="shrink-0 bg-white border border-red-200 text-red-600 hover:bg-red-50 px-6 py-2.5 rounded-[8px] text-[12px] font-bold transition-all shadow-sm">
                    Kosongkan Log
                  </button>
                </div>
              </div>
          ...
          {activeTab === "trash" && (
            <div className="bg-white border border-gray-200 rounded-[16px] shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
              <div className="p-6 sm:p-8 border-b border-gray-100">
                ...
                <p className="text-[13px] text-gray-500 mt-1">Pulihkan data mesin atau teknisi yang tidak sengaja terhapus.</p>
              </div>
              ...
              {isFetchingTrash ? (
                ...
                <div className="p-16 flex flex-col items-center justify-center text-center bg-gray-50/50">
                  ...
                  <h3 className="text-[15px] font-bold text-gray-900 mb-2">Tempat Sampah Kosong</h3>
                  <p className="text-[13px] text-gray-500 max-w-sm leading-relaxed">Belum ada data yang terhapus saat ini.</p>
                </div>
              ) : (
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {deletedMachines.map((machine) => (
                          <div key={machine.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-gray-200 rounded-[12px] bg-gray-50">
                            <div>
                              <p className="text-[14px] font-bold text-gray-900">{machine.nama_mesin}</p>
                              <p className="text-[12px] text-gray-500 mt-0.5">ID: {machine.product_id || "-"} • Kategori: {machine.kategori || "-"}</p>
                            </div>
                            <button 
                              onClick={() => triggerRestore(machine.id, "restore_machine", machine.nama_mesin)}
                              className="px-4 py-2 bg-white border border-gray-200 text-gray-700 text-[12px] font-bold rounded-[8px] hover:bg-gray-100 transition-colors flex items-center gap-2 shrink-0"
                            >
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {deletedTechs.map((tech) => (
                          <div key={tech.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-gray-200 rounded-[12px] bg-gray-50">
                            <div className="flex items-center gap-3">
                              {tech.foto_profil ? (
                                <img src={tech.foto_profil} alt={tech.nama_lengkap} className="w-10 h-10 rounded-full object-cover border border-gray-200" />
                              ) : (
                              ...
                              <div>
                                <p className="text-[14px] font-bold text-gray-900">{tech.nama_lengkap}</p>
                                <p className="text-[12px] text-gray-500 mt-0.5">{tech.role} • {tech.email}</p>
                              </div>
                            </div>
                            <button 
            ...
            {actionModal.status === "success" ? (
              ...
              <>
                ...
                <div className="flex justify-center gap-3">
                  <button onClick={() => setActionModal(prev => ({ ...prev, isOpen: false }))} className="px-6 py-2.5 text-[13px] font-bold text-gray-600 bg-gray-100 rounded-[10px] hover:bg-gray-200 transition-colors w-full">Batal</button>
                  <button onClick={executeAction} className={`px-6 py-2.5 text-[13px] font-bold text-white rounded-[10px] transition-colors w-full ${actionModal.type === 'clear_logs' ? 'bg-red-600 hover:bg-red-700' : 'bg-black hover:bg-gray-800'}`}>
