...
import { supabase } from "../utils/supabase"; // Pastikan path ini benar sesuai lokasi file lu
...
export default function Sidebar({ isSidebarOpen, setIsSidebarOpen }: SidebarProps) {
  ...
  const [isLoggingOut, setIsLoggingOut] = useState(false); // State buat nampilin loading pas logout
  ...
  const confirmLogout = async () => {
    setIsLoggingOut(true); // Nyalain efek loading

    try {
      // 1. Hancurkan tiket sesi di server Supabase
      const { error } = await supabase.auth.signOut();
      if (error) throw error;

      // 2. Bersihin semua cache di browser
      localStorage.removeItem("user_name");
      localStorage.removeItem("user_role");
      localStorage.removeItem("user_id");
      localStorage.removeItem("admin_avatar");

      // 3. Lempar balik ke halaman login (biar halaman otomatis reload dan Middleware baca session kosong)
          ...
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-[400px] p-8 text-center animate-in zoom-in-95 duration-200">
            ...
            {isLoggingOut ? (
              <div className="flex flex-col items-center justify-center py-4">
                <svg className="animate-spin h-10 w-10 text-[#E11D48] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                <h3 className="text-[16px] font-bold text-gray-900">Sedang Keluar...</h3>
